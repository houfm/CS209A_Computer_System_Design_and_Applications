...
public class OnlineCoursesAnalyzer {
   ...
   * concatenating the Institution and the course Subject (without quotation marks) using '-' while
  ...
  public Map<String, Integer> getPtcpCountByInstAndSubject() {
    Map<String, Integer> ans =
        courses.stream()
            .collect(
                Collectors.groupingBy(
                    Course::getInstAndSubject, Collectors.summingInt(Course::getParticipants)));
    ans =
        ans.entrySet().stream()
  ...
  public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
    ...
    for (String cur : avgAge.keySet()) {
      double sim =
          (age - avgAge.get(cur)) * (age - avgAge.get(cur))
              + (gender * 100 - avgGender.get(cur)) * (gender * 100 - avgGender.get(cur))
              + (isBachelorOrHigher * 100 - avgDegree.get(cur))
                  * (isBachelorOrHigher * 100 - avgDegree.get(cur));
